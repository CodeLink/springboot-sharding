...
public class HintTableShardingAlgorithm implements HintShardingAlgorithm<String> {
     ...
     * @param hintShardingValue    不再从SQL 解析中获取值，通过hintManager.addTableShardingValue("user", "1")参数指定
     * @return
     */
    @Override
    public Collection<String> doSharding(Collection<String> availableTargetNames, HintShardingValue<String> hintShardingValue) {
        ...
        if (result.isEmpty()) {
            throw new IllegalStateException(String.format("no table route with %s", hintShardingValue.getValues().toString()));
        }
        return result;
    }
}